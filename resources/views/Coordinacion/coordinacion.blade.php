...
<form action="{{route('crudcoord.createproveedor')}}" class="col 3 m-3" method="POST" enctype= "multipart/form-data" onsubmit="return validateForm()">
    ...
    <table id="personas" class="hover table table-striped shadow-lg mt-4 table-bordered table-hover">
      <thead style="background-color: #005E56;">
        <tr class="text-white">
          ...
          <th class="" scope="col">COORDINACIÓN</th>
...
return true;
 ...
 var table = $('#personas').DataTable({
  "ajax": "{{ route('datatable.proveedorcoordi') }}",
  "columns": [
    {
      data: null,
      title: '#',
...
{data: 'Observaciones'},
    {    data: null,
      render: function(data, type, row) {
        ...
        var url = "{{ route('crudcoor.update', ':id') }}";
