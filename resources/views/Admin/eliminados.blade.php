...
<form action="{{route('rol.store')}}" class="text-center" id="" method="POST" enctype="multipart/form-data" onsubmit="validateForm3()">
  @csrf
    ...
    <h2 class="p-2 mb-0 text-secondary text-start"><b>Registros Asociación Eliminados -
 ...
 var table = $('#personas').DataTable({
  ...
  render: function(data, type, row) {
    ...
    {    data: null,
      ...
      var deleteButton = '<a onclick="return eliminar2()" href="{{ route('crud.delete', ':id') }}" type="submit" class="btn btn-small btn-danger" name="eliminar" value="ok"><i class="fa-solid fa-trash"></i></a>'.replace(':id', row.ID);
