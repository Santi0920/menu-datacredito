...
<form action="{{route('crudmasivo.createproveedor')}}" class="col 3 m-3" method="POST" enctype= "multipart/form-data" onsubmit="return validateForm()">
...
return true;
 ...
 var table = $('#personas').DataTable({
  "ajax": "{{ route('datatable.proveedormasivo') }}",
  "columns": [
    {data: 'ID'},
    {data: 'TipoProveedor'},
    {data: 'NIT'},
...
{data: 'Observaciones'},
    {    data: null,
      render: function(data, type, row) {
        ...
        var url = "{{ route('crudmasivo.update', ':id') }}";
...
if(row.Consulta == 1){
  ...
  <div class="mb-3 w-100 text-start" title="Datacredito vencido!">
    ...
    <label for="Consulta" class="fw-semibold" style="font-size: 25px" onclick="toggleCheckbox()">SOLICITAR CONSULTA</label>
      ...
      var deleteButton = '<a onclick="showUnauthorizedMessage()" href="#" type="submit" class="btn btn-small btn-danger" name="eliminar" value="ok"><i class="fa-solid fa-trash"></i></a>';
